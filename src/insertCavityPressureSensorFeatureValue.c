...
int main(int argc, char *argv[])
{
	...
	//exeInsertMoldPressureFeatureValue -MOSN [2] -ShotSN [4]  [5] -MOSensorSN [7]
	if (argc == 8)
	{
	...
	for (int i = intShotSN_Start; i <= intShotSN_End; i++)
	{
		...
		{
			fprintf(stderr, "Fail to select mold pressure data for shotsn:%d\n", i);
			continue;
			//return EXIT_FAILURE;
		}
		...
		intRetval = DB_InsertCavityPressureSensorFeatureValue(mysqlCon, intMOSN, i, intMOCustomSensorSN, charCavityPressureSensorFeatureValue);
		if (intRetval != EXIT_SUCCESS)
		{
			fprintf(stderr, "Fail to insert to MySql server \n");
			continue;
			//return EXIT_FAILURE;
		}
	}
