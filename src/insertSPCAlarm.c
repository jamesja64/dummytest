...
int main(int argc, char *argv[])
{
	...
	if (argc == 6)
	{
		...
		{
			fprintf(stderr, "Format Error:insertSPCAlarm -MOSN [MOSN]\n");
			return EXIT_FAILURE;
		}
	...
	fprintf(stderr, "Insert SPC Rule Alarm  for MOSN:%d \n", intMOSN);
	...
	for (int i = intShotSN_Start; i <= intShotSN_End; i += 1){
		//intRetval = DEMO_InsertSPCAlarm(mysqlCon, intMOSN, i);
		//fprintf(stderr,"SPC Rule Alarm ShotSN:%d\n",i);
		intRetval = SPC_InsertSPCAlarm(mysqlCon, intMOSN, i);
		if (intRetval != EXIT_SUCCESS)
		{
			fprintf(stderr, "Fail to insert SPC Rule Alarm  for MOSN:%d ShotSN:%d\n", intMOSN,i);
		}
	}
